...
public class LinkedBinaryTree<T> implements BinaryTreeADT<T>, Iterable<T> {
    ...
    /**
     * Creates a binary tree with the specified element as its root and the given
     * trees as its left child and right child.
     *
     * @param theElement the element that will become the root of the binary tree
    ...
    public T getRootElement() throws EmptyCollectionException {
        ...
        if (isEmpty()) {
            throw new EmptyCollectionException("Tree is emply");
        }
...
//    public int sizeHelper(final BinaryTreeNode<T> theNode) {
//        if (theNode == null) {
//            return 0;
//        } else {
//            return sizeHelper(theNode.myLeft) + sizeHelper(theNode.myRight) + 1;
//            
//        }
//    }
    ...
    public int getHeight(final BinaryTreeNode<T> theNode) {
        ...
        if (theNode == null) {
            ...
            height = Math.max(getHeight(theNode.myLeft) + 1, getHeight(theNode.myRight) + 1);
        }
        return height;
    }
     ...
     * @return true if the element is in this tree, false otherwise
     */
    @Override
    ...
    /**
     * Returns a reference to the Node containing the specified target element if it
     * is found in this binary tree.
     *
     * @param theTargetElement the element being sought in this tree
     * @param theNode          the Node to begin searching from
     * @return a reference to the Node containing the target element if found; false
     *         otherwise
     */
    ...
    public Iterator<T> iteratorPreOrder() {
        // To be completed as a Programming Project
        final ArrayUnorderedList<T> preList = new ArrayUnorderedList<T>();
        preOrder(myRoot, preList);

        return new TreeIterator(preList.iterator());
    }
    ...
    public Iterator<T> iteratorPostOrder() {
        // To be completed as a Programming Project
        final ArrayUnorderedList<T> temList = new ArrayUnorderedList<T>();
        postOrder(myRoot, temList);
        return new TreeIterator(temList.iterator());
    }
    ...
    /**
     * support to find count of leaf.
     * 
     * @param theNode node of binary tree
     * @return the number of leaf
     */
    private int countLeafNodes(final BinaryTreeNode<T> theNode) {
        int count = 0;
        if (theNode == null) {
            throw new EmptyCollectionException("Tree node is empty");
        }
        if (theNode.myRight == null && theNode.myLeft == null) {
            count++;
        } else {
            count = countLeafNodes(theNode.myLeft) 
                               + countLeafNodes(theNode.myRight);
        }
        return count;
    }
